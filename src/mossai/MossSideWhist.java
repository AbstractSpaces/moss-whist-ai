...
public class MossSideWhist
{
    ...
    public MossSideWhist(MSWAgent p1, String n1, MSWAgent p2, String n2, MSWAgent p3, String n3)
    {
        ...
        p1.setup(left, right);
        p2.setup(right, leader);
        p3.setup(leader, left);
    }
    ...
    {
        String second = left;
        String third = right;//calculate the position of each player.
    ...
    {
        if(lead.compareTo(next) < 0 || !followsSuit(lead, next))
        {//first beats second
            if(lead.compareTo(last) < 0 || !followsSuit(lead, last))
            {//first beats third
                return first;
            }
            else
        ...
        {//second beats first
            if(next.compareTo(last) < 0 || !followsSuit(lead, last))
            {//second beats thirs
                return second;
            }
            else
        ...
        if(hands.get(player).contains(played))
        {
            if(played.suit != suit)
            {
                for(Card c : hands.get(player))
                {
                    if(c.suit == suit)
                    {
                        return false; // card didn't match suit, but had one in hand.
                    }
                }
